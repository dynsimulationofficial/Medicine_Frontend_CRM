...
import { MdOutlineSettings, MdDateRange } from "react-icons/md";
...
export default function LeadSourcesPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await AxiosProvider.get(`/lead-sources?page=${page}&limit=20`);
      if (res.data?.success) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (res.isConfirmed) {
      try {
        await AxiosProvider.delete(`/lead-sources/${id}`);
  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    try {
      ...
      const res = isEdit
        ? await AxiosProvider.put(`/lead-sources/${selectedData.id}`, values)
        : await AxiosProvider.post("/lead-sources", values);

      if (res.data?.success) {
        toast.success(isEdit ? "Lead source updated successfully" : "Lead source created successfully");
        closeFlyout();
        fetchData();
      }
    } catch (error: any) {
      ...
      <div className="flex justify-end min-h-screen">
        <div className="ml-[97px] w-full md:w-[90%] m-auto min-h-[500px] rounded p-4 mt-0">
          ...
          <div className="relative overflow-x-auto shadow-lastTransaction rounded-xl sm:rounded-3xl px-1 py-6 md:p-6 z-10 mainContainerBg">
            {/* Top Action Button (Create Lead Source) */}
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-white">
                      <div className="animate-pulse">Loading lead sources...</div>
                    </td>
                  </tr>
                ) : data.length === 0 ? (
                  <tr>
          ...
          <div className="w-full min-h-auto p-6 sm:p-8 text-white">
            <div className="flex justify-between items-center mb-6">
              <p className="text-primary-500 text-[26px] font-bold leading-9">
                {flyout === "edit" ? "Edit Lead Source" : "Create Lead Source"}
              </p>
              <IoCloseOutline
                onClick={closeFlyout}
              ...
              {({ isSubmitting }) => (
                <Form className="space-y-4">
                  <div>
                    ...
                    <Field
                      name="name"
                      placeholder="e.g. Facebook, Google Ads, WhatsApp"
                  ...
                  <div className="pt-4">
                    ...
                    >
                      {isSubmitting ? "Submitting..." : flyout === "edit" ? "Update Source" : "Create Lead Source"}
                    </button>
                  </div>
                </Form>
              )}
        ...
        {flyout === "view" && selectedData && (
          <div className="w-full min-h-auto p-6 sm:p-8 text-white">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-primary-400 border-b border-gray-800 pb-2">Source Information</h3>
